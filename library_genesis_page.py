...
def enter_query():
    ...
    i_page = input('enter page number: ')
    i_page = int(i_page)
    compile_ids(i_page, f_dir, search_q)


def compile_ids(i_page, f_dir, search_q):
    ...
    for _ in lookup_ids:
        try:
            ...
            if ext:
                ...
                save_path = save_path + ' (by ' + "".join(
                    [c for c in author if c.isalpha() or c.isdigit() or c == ' ']).rstrip()
                save_path = save_path + ' ' + "".join(
                    [c for c in year if c.isalpha() or c.isdigit() or c == ' ']).rstrip() + ')' + ext
